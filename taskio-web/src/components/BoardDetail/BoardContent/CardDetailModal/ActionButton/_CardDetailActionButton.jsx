...
function CardDetailActionButton({ data, handler, activeCard }) {
  ...
  return (
    ...
    >
      ...
      <CardAIAssistPopover
        handleGenerate={(userPrompt) => handleGenerateAIAssist(activeCardRef.current, userPrompt)}
        handleApply={(payload) => handleApplyAIAssist(payload, activeCardRef.current)}
      />
    </Box>
  )
}
